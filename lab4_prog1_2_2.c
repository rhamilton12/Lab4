...
void *image_capture( void* arg5)
{
	...
	while (true) {
			...
			imwrite("/media/card/capture_Image/image.jpg", cameraFrame);
...
void *client_server(void* arg6)
{
	...
	char buf[1024];
	snprintf(buf, 1024, "http://%s:%d/update?id=%d&password=%s&name=%s&data=%d&status=%s&buff=%s&filename=%s", hostname, port, id, password, name, adcval, status, buff, filename);
	FILE *fp;
	 ...
	 while(1)
      {
		...
		printf("	  	1 -> Reset_ADC \n");
		printf("      		2 -> Ping_PIC \n");
		printf("      		3 -> Get Voltage value \n");
		printf("      		4 -> Get Temperature value \n");		
		...
		if (command == MSG_RESET)
		{
			pthread_t pt1;
            int ret1;
			//pthread_cancel(PTHREAD_CANCEL_DISABLE, &pt1);
			ret1 = pthread_create(&pt1, NULL, &reset, NULL);
			if(ret1 != 0) 
				{
		...
		else if (command == MSG_PING)
		{
			...
			ret2 = pthread_create(&pt2, NULL, &ping, NULL);
			if(ret2 != 0) 
				{
					printf("Error: pthread_create() failed\n");
					exit(EXIT_FAILURE);
		...
		else if (command == GET_VOLT)
		{
            ...
            int ret3;
			//pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);			
			ret3 = pthread_create(&pt3, NULL, &get, NULL);
			if(ret3 != 0) 
				{
					printf("Error: pthread_create() failed\n");
					exit(EXIT_FAILURE);
		...
		if (command == GET_TEMP)
		{
            ...
            int ret4;
			//pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);			
			ret4 = pthread_create(&pt4, NULL, &temperature_reading, NULL);
			if(ret4 != 0) 
				{
					printf("Error: pthread_create() failed\n");
					exit(EXIT_FAILURE);
		...
		else if (command == GET_PICT)
		{
            ...
            int ret5;
			//pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);			
			ret5 = pthread_create(&pt5, NULL, &image_capture, NULL);
			if(ret5 != 0) 
				{
					printf("Error: pthread_create() failed\n");
					exit(EXIT_FAILURE);
		...
		else if (command == SEND_DATA)
		{
            ...
            int ret6;
			//pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);			
			ret6 = pthread_create(&pt6, NULL, &client_server, NULL);
			if(ret6 != 0) 
				{
					printf("Error: pthread_create() failed\n");
					exit(EXIT_FAILURE);
...
//compiling command
//root@galileo:~# g++ -lpthread -lopencv_highgui -lopencv_imgproc -lopencv_objdetect -lopencv_core -lcurl lab4_prog1.c -o lab4_2