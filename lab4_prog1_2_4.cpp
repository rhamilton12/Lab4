...
#define VREF	(3303.3) //Voltage reference in mVolts
#define THRESHOLD  0x7
...
void *get(void *arg3)
{
		...
		setGPIO(STROBE, GPIO_DIRECTION_OUT);
			...
			volt = (results * VREF)/1023 ;
			printf("ADC reults: %.2f mV\n",volt);		
}
...
void *client_server(void* arg6)
{
	...
	const char* hostname = "192.168.43.242";  //server IP address 192.168.43.242
	const int   port = 8000;
	const int   id = 8;
	const char* password = "password";
	const char* name = "GP_8";
	const char* status = "Very+good+thank+you";
	const char* filename = "G8_image.jpg";
	double adcval = *voltptr; //254
   ...
   //printf("Current local time and date: %s", asctime(info));
	...
	char buf[1024];
	snprintf(buf, 1024, "http://%s:%d/update?id=%d&password=%s&name=%s&data=%lf&status=%s&asctime(info)=%s&filename=%s",
			hostname,
			port,
			id,
			password,
	 ...
	 while(1)
      {
		...
		printf("      		7 -> Set the temperature Threshold \n");
		scanf("%d", &command);
		 ...
		 else if (command == THRESHOLD)
		{
			...
			if (thrs <= *tem_ptr)
				{
					cout<<"Taking the picture";
					pthread_t pt8;
					int ret8;	
					ret8 = pthread_create(&pt8, NULL, &image_capture, NULL); //thread8 creation
					if(ret8 != 0) 
					{
						printf("Error: pthread_create() failed\n");
						exit(EXIT_FAILURE);
...
//compiling command
//root@galileo:~# g++ -lpthread -lopencv_highgui -lopencv_imgproc -lopencv_objdetect -lopencv_core -lcurl lab4_prog1_2_4.cpp -o lab4_3
//root@galileo:~# g++ -lopencv_highgui -lopencv_imgproc -lopencv_objdetect -lopencv_core lab3_imagecapture.cpp -o run
